import { r as registerInstance, h, g as getElement } from './index-9a244d12.js';
...
const canvas$2 = /*@__PURE__*/getDefaultExportFromCjs(canvas$1);
...
var _canvas2 = _interopRequireDefault(canvas$1);
...
class SetImage extends AbstractSetter {
  async run() {
    return new Promise((resolve, rejects) => {
      const imgData = new Image();
...
class SetText extends AbstractSetter {
  async run() {
...
async function canvasSetter(canvas, layer) {
  const mapeador = {
...
const imageCanvasCss = "image-canvas{display:block}";
...
const ImageCanvas = class {
  ...
  render() {
    if (this.layers.length == 0) {
      return h("p", { style: { "color": "red" } }, "ERR: Debe ejecutar $(\"image-cangas\").setLayers(Arr con layers[])");
    }
  }
